...
{
    /* Load the core OpenCL stuff. We do it before anything else, so that if it's
     * not supported, we'll fail early.*/
    cl_int err;
    cl_uint pcnt;
    ...
    if ( !strstr( extstr, "cl_khr_gl_sharing" ) )
        xplat_error( "Compiled for the OpenCL \"cl_khr_gl_sharing\" extension, but "
                     "it's not supported by the device." );
#endif
}
...
void init_cl_post( void )
{
    ...
    {
        CL_CONTEXT_PLATFORM,
        (cl_context_properties)engine.cl.platform,
#if !defined( NOSHARING )
#if defined( __linux__ )
        CL_GL_CONTEXT_KHR,
        ( cl_context_properties )( glfwGetGLXContext( engine.gl.window ) ),
        CL_GLX_DISPLAY_KHR,
        ( cl_context_properties )( glfwGetX11Display() ),
#elif defined( _WIN32 )
        CL_GL_CONTEXT_KHR,
        ( cl_context_properties )( glfwGetWGLContext( engine.gl.window ) ),
        CL_WGL_HDC_KHR,
        ( cl_context_properties )( GetDC( glfwGetWin32Window( engine.gl.window ) ) ),
#endif
#endif
        0,
        0,
    ...
    engine.cl.texture =
        clCreateImage2D( engine.cl.context, CL_MEM_WRITE_ONLY | CL_MEM_ALLOC_HOST_PTR, &format, engine.cl.dim[0], engine.cl.dim[1], 0, NULL, &err );
    CLFCHECK( err );
#else
    ...
    char opt[256] = {0};
    // Some extra optimizations, for testing: -cl-single-precision-constant
