...
static void render_cl( void )
{
    ...
    Camera camera = {
        .pos = {8.1, 8.1, 4.3}, .trg = {cos( rt ) * 2.0, sin( rt ) * 2.0, 0.0}, .nor = {0.0, 0.0, 1.0}, .aspect = ww / (double)wh, .lens = 1.0,
    };
    ...
    if ( !test )
    {
        OTnode_cl branches[6] = {{.p = 1}, {.p = 2}, {.p = 3}, {.p = 4}, {.p = 5}, {.p = 0}};
        branches[0].bf = 1;
        branches[1].bf = 1;
        branches[2].bf = 1;
        branches[3].bf = 1;
...
/* Acquire OpenGL texture (if applicable) & call the core kernel */
    ...
    CLFCHECK( err );
#else
    const uint32_t nb = engine.cl.dim[0] * engine.cl.dim[1] * 4;
    const size_t origin[3] = {0, 0, 0};
    const size_t region[3] = {engine.cl.dim[0], engine.cl.dim[1], 1};
    uint8_t *tdata = malloc( nb );
    err = clEnqueueReadImage( engine.cl.queue, engine.cl.texture, CL_TRUE, origin, region, 0, 0, tdata, 0, NULL, NULL );
    CLFCHECK( err );

    glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA, (int)engine.cl.dim[0], (int)engine.cl.dim[1], 0, GL_RGBA, GL_UNSIGNED_BYTE, tdata );
    free( tdata );
#endif
