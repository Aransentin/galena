...
{
	double dir[3] = {
		camera->trg[0] - camera->pos[0],
		camera->trg[1] - camera->pos[1],
		camera->trg[2] - camera->pos[2],
	};
	normalize( dir );
	camera->dir[0] = dir[0];
	camera->dir[1] = dir[1];
...
void camera_cl_update( Camera * camera )
{
	...
	const double frstC[3] = {
		camera->lens * camera->dir[0],
		camera->lens * camera->dir[1],
		camera->lens * camera->dir[2],
	};
	...
	const double frstS[3] = {
		0.5 * side[0] * camera->aspect,
		0.5 * side[1] * camera->aspect,
		0.5 * side[2] * camera->aspect,
	};
	
	const double vo[3] = {
		frstC[0] - frstN[0] - frstS[0],
		frstC[1] - frstN[1] - frstS[1],
		frstC[2] - frstN[2] - frstS[2],
	};
